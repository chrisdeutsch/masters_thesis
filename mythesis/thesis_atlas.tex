...
\begin{figure}[htb]
  ...
  \caption{Overview of the ATLAS inner detector~\cite{indet_fig}. Shown are
    ...
    The innermost pixel layer, the Insertable B-Layer, is not depicted.}
...
individual cells of the calorimeter calibrated at EM scale, therefore not
accounting for the different calorimeter response to hadronic showers depositing
the same energy as an electron or photon. The cluster formation is seeded by the
cell of largest signal significance~\cite{atlas_topoclustering}
\begin{align*}
...
The local hadronic calibration (LC), described in~\cite{}

Accurate cluster energies independent of the physics object that created it.
Important for objects with significant hadronic signal content

Topocluster moments provides information on the object that created the cluster

Topocluster->EMprobability->calibrate EM--HAD->out of cluster corrections->dead
material corrections

Cluster-by-cluster moment dependent calibration (non-compensating nature)

Energy loss due to TopoCluster noise suppression (lost cells)

Energy loss due to dead material

Probability of being a EM cluster from cluster moments

Separate weights for EM and HAD calibrations according to probability w-cal =
pem * w-em + (1-pem) w-hadcal

EM scale already has out of cluster and dead material corrections
