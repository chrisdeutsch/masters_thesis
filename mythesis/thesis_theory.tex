...
\subsubsection{Standard Model Higgs Boson in the \tauhad\tauhad Channel}
...
of~$H \to \tau\tau$ decays, which is only marginally exceeded
by~$\taulep\tauhad$. The~\tauhad are susceptible to being faked by jets
...
Discriminating variable $m_{\tau\tau}^\text{MMC}$~\cite{mmc}
...
In Figure~\ref{fig:mtautau_mmc}

- One of to categories: VBF mostly production using vector boson fusion
(presence of two jets with high distance in pseudorapidity)

- Requiring two identified \tauhad and additional isolation cuts to reduce multijet background

Aside from large $Z \to \tau\tau$ background, still significant fake \tauhad
background exceeding the signal contribution.




Higgs $\mathcal{CP}$ hadronisation washes out polarisation.
