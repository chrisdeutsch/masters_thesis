...
the Standard Model. \todo{Introduce Yukawa coupling proportional to mass}
...
vast majority of taus decays are 1- or 3-prong. \todo{intermediate meson
  resonances? $h^- \pi^0 \nu_\tau$ -- $\rho(770)$, Modes with 3 pions
  $a_1(1260)$}
...
Tau leptons are important for the physics programme of the ATLAS and CMS
experiments at the LHC.

\subsubsection{Standard Model Higgs in the Ditau Channel}
...
Only channel $H \to \tau \tau$ and $h \to b \bar{b}$ but $b \bar{b}$ difficult
due to high background (requires associated production of the higgs with vector
boson drastically limiting the rate but still large backgrounds). $\tau\tau$
favourable sensitivity.

Two categories: VBF (presence of two jets with high distance in pseudorapidity) and Boosted (Large transverse momentum higgs -- gluon fusion with additional radiated gluons)

Three channels: leplep, lephad, hadhad

Focus hadhad:

Requiring two identified \tauhad candidates. Additional cuts to reduce multijet background.

Ditau invariant mass $m_{\tau\tau}^\text{MMC}$ Missing Mass Calculator (MMC)
uses constraints from the measured transverse components of the missing
transverse energy and the visible mass of both tau leptons to determine the
likely invariant mass of the Higgs

Significant background: $Z \to \tau\tau$, multijet, $W+\text{jets}$, $t\bar{t}$
(small)

ditau invariant mass is the discriminating variable


\subsubsection{Search for New Heavy Neutral Higgs and Gauge Bosons}
