...
This effect has a large impact on
the overall isolation of the shower in the calorimeter and the tracks.
An example of this is the central energy fraction $f_\text{cent}$ shown in
figure [does not exist anymore]. While the energy for signal events is mostly
...
low $p_\text{T}$ tau candidates.

To take advantage of the varying signal/background separation of variables in
different $p_\text{T}$-regions the reconstructed transverse momentum can be
included as an input to the BDT. Correlations between the remaining input
variables and the reconstructed~$p_\text{T}$ can then be used to discriminate
signal and background tau candidates. The kinematic reweighting (cf.\
...
systematic errors of the tau energy scale.
The BDT uses intermediate axis
...
\begin{figure}[ht]
  ...
  \caption{Variable importance. Averaged rejection loss over a gamma-tautau like
    dijet spectrum}
  \todo[inline]{3-prong uses medium working point.}
