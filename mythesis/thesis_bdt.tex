...
momentum~\smash{$p_\text{T}^\text{clamp}$} in both BDTs and the momentum
fraction of isolation tracks~\smash{$f_\text{iso}^\text{track}$} in the 3-prong
...
In Figure~\ref{fig:variable_importance} the variable importance for the 1- and
...
\todo{Say something about $p_\text{T}^\text{clamp}$ and
...
(\smash{$p_\text{T}^\text{EM+track} / p_\text{T}$}) with a linear correlation
...
fraction of charged pions~\smash{$f_\text{EM}^\text{track-HAD}$}, due to its
correlation with~\smash{$p_\text{T}^\text{EM+track} / p_\text{T}$}, contributes
...
\todo[inline]{Impact on (reconstructed) decay modes (?)}
