...
Machine learning algorithms have hyperparameters that define the complexity of
the model. Hyperparameters cannot be determined as part of the training process
and must be set beforehand. Often these parameters need problem specific tuning
to obtain the highest discriminative power for a given algorithm. The BDTs of
the 1- and 3-prong tau identification are optimised separately with respect to
the boosting algorithm used and the most important hyperparameters.
...
applied to the input variables to improve the split finding. For this highly
skewed distributions are log-transformed while long-tailed distributions and
outliers due to detector resolution effects are clamped to fall into a specified
range of values. The exact transformations can be found in
...
\todo[inline]{Subsection of 'Hyperparameter Optimisation' makes no sense!}
