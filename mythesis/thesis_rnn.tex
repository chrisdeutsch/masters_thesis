...
\item[Impact parameters w.r.t.\ the tau vertex] The transverse~$d_0$ and
  ...
  in the $\rho z$-plane) with respect to the associated primary vertex of the tau
...
\begin{figure}[htb]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \subcaption{Distance of closest approach to the tau vertex in the
...
variables for each track in the sequence with the output of the
Track-RNN~$p_\text{RNN}$ is depicted.

- Typically the leading or three leading tracks are tracks originating from the tau decay

- Compared to dijet events the leading or leading three tracks of signal show a significant increase in transverse momentum

- High pt tracks after the leading / three leading tracks are more likely associated with background events

- Similarly leading tracks close to the primary vertex. Increasing distance leads to a smaller signal probability.

- $|d_0|$ same behaviour

- A larger distance of the subleading tracks results in higher signal
probabilities.

- The angular distances $\Delta \eta$ $\Delta \varphi$ similar to~$z_0$
