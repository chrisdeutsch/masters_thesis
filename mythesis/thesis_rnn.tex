...
identification from \emph{isolation} and \emph{fake} tracks by introducing the
\emph{modified isolation} category (cf.\
...
tracks. Figure~\ref{fig:ntracks_loss} shows the validation loss for different
numbers of tracks used in the model. The first six tracks show a significant
decrease in validation loss, therefore improving the discriminative power of the
model. Hardly any further improvement is observed after ten tracks. Therefore,
the model only uses the ten leading tracks in~$p_\text{T}^\text{track}$.
...
\begin{figure}[htb]
  ...
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.48\textwidth}
    \centering \includegraphics{./figures/rnn/nscan/track_1p_3p.pdf}
  ...
  \end{subfigure}
  \caption[Number of tracks associated to \tauhadvis candidates and their impact
  on the discriminative power of tau identification]{Number of tracks associated
    to a \tauhadvis candidate and their impact on the discriminative power of
    the Track--RNN.}
  \label{fig:rnn_ntracks}
\end{figure}

A comparison of the ROC-curves for the BDT-based identification using track,
vertex and calorimeter information, and the RNN-based identification using
tracks is depicted in Figure~\ref{fig:track_rnn_roc_ratios}. Both the 1- and
3-prong Track--RNN show performances similar to the optimised BDT, despite using
only tracking information and~$p_\text{T}^\text{jet}$. The 1-prong Track--RNN
...
\begin{figure}[htb]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics{./figures/rnn/track/ratios_fixed.pdf}
...
\begin{figure}[htb]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics{./figures/rnn/cluster/ratios_fixed.pdf}
...
An upgrade of the ATLAS trigger with a Global Trigger System aims to
...
the purposes of this study a fixed cut on the identification score is used to
identify ditau events.
...
transverse momenta of~$p_\text{T}^\text{true} > \SI{40}{\GeV}$ of the leading
...
\begin{figure}[htb]
  ...
  \end{subfigure}
  \caption[Performance of the TopoCluster-based ditau
  identification]{Performance of the TopoCluster-based ditau identification for
    use in a ditau trigger at the HL-LHC.}
...
\begin{figure}[htb]
  ...
  \caption[Network architecture of a model combining high-level identification
  variables, tracks and clusters]{Architecture of the combined model operating
    on high-level identification variables and sequences of tracks and clusters.
    The activation function is denoted after each layer (cf.\
...
\begin{figure}[htb]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics{./figures/rnn/combined/ratios_fixed.pdf}
