...
- New trigger architecture introducing a 'Global Trigger System' using full calorimeter granularity with topoclustering and anti-kt jets (to improve object identification which is essential in high pile-up scenario)

Initial scoping document: Offline thresholds \SI{40}{\GeV}, \SI{30}{\GeV}
(approx. run I threshold) in $|\eta| < 2.5$~\cite{phase_2_scoping} with output
rate of \SI{200}{\kilo\hertz}

No upgrade \SI{60}{\GeV}, \SI{50}{\GeV} \cite{phase_2_idr}
...
- Investigation using offline reconstructed taus (upgrades in readout allow anti-kt jets at trigger and therefore taus are close to what we have at offline)

- Preselection the same except no tracking related cuts (i.e.\ no separation in 1 and 3-prong decays)
