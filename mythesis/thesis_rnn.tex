...
\item[Cluster and signal moments] Identification variables like the central
  ...
  cluster and signal moments are implemented in the TopoCluster
...
electromagnetic
shower~$\mathcal{P}_\text{clus}^\text{EM}$~\cite{atlas_topoclustering} used to
apply the local hadronic calibration are investigated. These variables are not
found to improve the classification power of the cluster-based identification.
...
\begin{figure}[htb]
  ...
  \caption{Architecture of the combined model operating on high-level
    ...
    activation function is denoted after each layer. Layers operating on
    sequential inputs are highlighted in blue.}
...
\todo[inline]{Write sth.\ about Track+Cluster}
...
Figure~\ref{fig:combined_working_points} shows the rejection of the tight
working point as a function of the \tauhadvis \pt.
...
comparable to the optimised BDT. For example the signal efficiency of the tight
working point for the 1-prong identification can be increased from
\SI{60}{\percent} to \SI{70}{\percent}. In doing this the rejection of fake
\tauhadvis candidates with transverse momentum of \SI{20}{\GeV} matches the
rejection of the BDT using the working point with the nominal efficiency. For
larger transverse momenta the rejection of the RNN at the loosened working point
still exceeds the BDT-based identification. Analogously, the signal efficiency
of the 3-prong tight working point can be increased from \SI{45}{\percent} to
\SI{50}{\percent}.

For $H \to \tauhad \tauhad$ analyses requiring two tight \tauhadvis this would
result in an relative increase of 1-prong signal events of \SI{36}{\percent} and
3-prong signal events of \SI{23}{\percent}, assuming uncorrelated



- 1-prong: Increase tight working point efficiency from 60 to 70 percent
- roughly same rejection at low pt
- still better at high pt

- 3-prong: Increase in tight working point efficiency from 45 to 50 percent
- roughly same rejection at low pt
- still better at high pt
...
% 0.6 * 0.6 = 0.36
% 0.7 * 0.7 = 0.49
% 0.49 / 0.36 = 1.3611111111 -> 36 % relative increase in signal for $H \to \tauhad \tauhad$

pp% 3-prong
% 0.45 * 0.45 = 0.2025
% 0.5 * 0.5 = 0.25
% 0.25 / 0.2025 = 1.234567901234568 -> 23 % relative increase in signal for $H \to \tauhad \tauhad$

- Multiplicity increase of dijets at high transverse momentum could lead to
better rejection \cite{jet_multiplicity}
...
\todo[inline]{Still improving when including calorimeter information.}
