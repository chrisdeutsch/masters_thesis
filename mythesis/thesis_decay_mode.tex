...
\todo[inline]{Shots are actually used in PanTau. Check if I stated the opposite.}
...
passed through a network containing three dense layers. The final dense layer
has five units equal to the number of decay modes to classify. Application of
the Softmax activation function on the final layer ensures that the output
activations can be interpreted as mode probabilities.
...
intermediate~\smash{$\text{K}^0$} are used. The mode composition of the sample
...
objects. The variables attached to each neutral and charged PFO are summarised
in the following:
\begin{description}
...
subsequently standardised. \todo{by subtracting the mean and dividing by the
  standard deviation, derived from the distributions in the training sample.}
For the neutral PFO specific variables~\smash{$S_\text{BDT}^{\pi^0}$} and
...
intermediate representations after the shared dense layers are passed into the
...
pions for low momentum tau decays.
