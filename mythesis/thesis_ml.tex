...
weights~$w_i$ the loss~$\mathcal{L}$ is defined as \todo{Somehow denote subset of data}
\begin{align*}
...
\todo{Citation}. The training process minimises the loss~$\mathcal{L}$
...
\end{align*}
where $\eta > 0$ is called the learning rate and \todo{Gradient on the
  mini-batch (stochastic approximation)}. One pass over all mini-batches is
...
\todo[inline]{Find more references} \cite{esl}
