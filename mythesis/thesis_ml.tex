...
used for feedforward neural networks but now also considering that the
chain-rule has to be applied for each time step and one has to consider that the
weights between time steps are shared (\emph{backpropagation through time}
\cite{williams_zipser}).

For sequence classification only the last output vector is used.

Vanishing or exploding gradient problem: Applying the sigmoid function multiple
times leads to vanishing gradients (plot?). \cite{lecun_bengio_hinton_DL}

Empirical evidence shows that it is difficult to store information long-term
\cite{lecun_bengio_hinton_DL}.
