...
\begin{figure}[htb]
  ...
  \begin{minipage}[t]{0.4\textwidth}
    ...
    \captionof{figure}[Commonly used activation functions]{Commonly used activation
...
\begin{figure}[htb]
  ...
  \caption[LSTM-cell unfolded in time]{LSTM-cell unfolded in time at a fixed
    time step~$t$. Nodes with~$\times$ ($+$) multiply (add) their inputs
    element-wise, while ones containing a sigmoid shape apply a nonlinear
    activation function. The dependency of the gate vectors $\mathbf{i}_t$,
    $\mathbf{o}_t$ and $\mathbf{f}_t$ on input~$\mathbf{x}_t$ and
    output~$\mathbf{y}_{t-1}$ is omitted.}
